...
async function hGetState(env,uid,tg,data={},_meta={}){
  try{
    ...
    // Update leaderboard entry AFTER user is guaranteed to exist in DB
    ...
    const depr=await dbGet(env,`users/${uid}/deposits`);
...
async function updateLeaderboardEntry(env,uid,user){
  try{
    ...
    if(!meta||!meta.endDate||!meta.startDate){
      // Only create if truly missing — never overwrite existing dates
      meta={startDate:meta?.startDate||nowMs, endDate:meta?.endDate||(nowMs+COMP_DURATION_MS)};
      await dbSet(env,'competition/meta',meta);
    }
    // Auto-renew if competition ended
    if(nowMs>meta.endDate){
      meta={startDate:nowMs,endDate:nowMs+COMP_DURATION_MS};
      await dbSet(env,'competition/meta',meta);
      await dbSet(env,'competition/snapshots',null);
      await dbSet(env,'competition/users',null);
      await dbSet(env,'competition/leaderboard',null);
    }
...
async function hGetLeaderboard(env,uid,_meta={}){
  try{
    ...
    if(!meta||!meta.endDate||!meta.startDate){
      meta={startDate:meta?.startDate||nowMs2, endDate:meta?.endDate||(nowMs2+COMP_DURATION_MS)};
      await dbSet(env,'competition/meta',meta);
    }
    if(nowMs2>meta.endDate){
      meta={startDate:nowMs2,endDate:nowMs2+COMP_DURATION_MS};
      await dbSet(env,'competition/meta',meta);
    }
...
// ── Season Allocation ─────────────────────────────────────────────
async function hSaveSeasonAlloc(env,uid,data={}){
  try{
    ...
    await dbSet(env,`season2/alloc/${uid}`, rec);
    return{success:true};
  }catch(e){return{success:false,error:e.message};}
}

async function hGetSeasonAlloc(env,uid){
  try{
    const r=await dbGet(env,`season2/alloc/${uid}`);
...
export default {
  async fetch(request,env){
    ...
    switch(action){
      ...
      case 'saveSeasonAlloc':return jRes(await hSaveSeasonAlloc(env,uid,data));
      case 'getSeasonAlloc' :return jRes(await hGetSeasonAlloc (env,uid));
